...
The public CI workflow builds and tests every pull request plus pushes to `main` and `dev`. The `dev` branch is the source for the automatically refreshed `dev` GitHub prerelease, with versioned installer, portable assets, and notes from the `Unreleased` changelog section. The `main` branch remains the stable source: stable releases require `Version.h` to use the `stable` channel, a matching changelog section, and a matching tag such as `v0.1.4`. The stable workflow publishes the installer, portable ZIP, checksums, signed update manifest, signature, and curated changelog notes. With developer updates enabled, the updater checks both signed feeds; stable releases remain eligible and take precedence over developer prereleases when appropriate, while developer prereleases never replace `releases/latest`.
